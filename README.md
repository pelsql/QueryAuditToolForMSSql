...
- **Version 2.7** : 
- Before upgrading to version 2.7, it is important to run the `PrepareSwitchPre2.7VersionToEnglish` script, which migrates the French version to anglicized code. The purpose of this anglicization is to make the open-source code more accessible to a wider audience. Do not run it, if it is a new install equal or above version 2.7.
- Version 2.7 also includes significant changes due to a bug in the command `SELECT create_date FROM sys.dm_xe_sessions`, which returns a `create_date` value that drifts over time. Fortunately, the solution simplifies the code. This date has been replaced by the date of the first event in a trace file when the `event_sequence` is 1. This provides performance benefits to the logon trigger, as it no longer queries `sys.dm_xe_sessions` for each login. It also no longer transfers this value into the `user_event` it generates, and it no longer needs to be retrieved by the procedure that combines login information with queries.
