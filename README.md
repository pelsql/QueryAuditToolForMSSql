...
- **Version 2.8**:  
  ...
  - This version introduces the ability to store administrative settings, making it unnecessary to reinitialize them during future updates on servers where the script has already been executed. The goal is to simplify updates by enabling users to retrieve and execute the latest script without needing to reconfigure administrative settings. Refer to #YourConfig near the beginning of the script and the explanatory comment above for more details.
...
- **Version 2.5.0**:  
  - Many fixes to problems found under heavy load, and other resiliency features.

---

- **Version 2.1.2**:  
  - RPC_Completed events added.

---

- **Version 2.1.1**:  
  - Streamlining of a process (minor modification) and adjustments to tests.

---

- **Version 2.1**:  
  - Enhanced resilience to prevent event loss in case of interruption.  
  - Added a mechanism to prevent duplicate entries and implemented an automatic task restart every 15 minutes in case of stoppage.  
  - Support for multiple connections per session.  
  - Improved testing tools to simplify the verification of result accuracy, ensuring that no events are missing.

---

- **Version 2.0**:  
  - Eliminated the need to store connection information in a table at the login trigger level, replacing the mechanism with the use of a user event containing the necessary information.  
  - Events are now sequenced to associate queries with their session number and event sequence. This method avoids the difficulties of linking by rounded temporal data at the trace file level.
...
- **Version 2.7** :  
  - Avant de passer à la version 2.7, il est important de faire exécuter le code `PrepareSwitchPre2.7VersionToEnglish`, qui migre la version française à du code anglicisé. Le but de cette anglicisation est de rendre ce code ouvert plus accessible à un large auditoire. NE PAS l'exécuter si c'est une nouvelle installation dont la version est égale ou supérieure à 2.7.  
  - La version 2.7 fait aussi l'objet de modifications importantes, dues à un bug de la commande `Select create_date From Sys.Dm_XE_Sessions`, qui retourne une valeur de `create_date` qui dérive avec le temps. Heureusement, la solution simplifie le code. Cette date a été remplacée par la date du premier événement d'un fichier de trace lorsque `event_sequence` est 1. Cela donne des avantages de performance au Logon trigger, qui ne consulte plus `Sys.Dm_XE_Sessions` à chaque login. Il ne transfère plus cette valeur dans l'événement utilisateur généré et n'a plus besoin d'être récupérée par la procédure qui combine les informations de login aux requêtes.  
  - D'autres ajustements ont été faits pour rendre le code plus facile à diagnostiquer en cas de problème, notamment grâce à des courriels informatifs et des tests de conformité d'environnement réguliers.
...
- **Version 2.6.1** :  
  - Ajustements pour la migration directe des versions antérieures à 2.60 vers 2.61. Certains de ces ajustements seront retirés, car un seul client a des versions antérieures à 2.60.  
  - Correctif mineur visant à résoudre un problème où certaines connexions SQL Server Agent restent souvent ouvertes depuis une période prolongée, précédant même l’installation ou la mise à jour du produit. Ces connexions ne sont pas détectées par le logon trigger, car celui-ci ne déclenche pas d’événements de connexion si la session Extended Events n’est pas active. Dans ces cas, on peut observer les connexions dans la vue des sessions actives. En effet, si elles n’ont pas été tracées, c’est qu’elles étaient déjà ouvertes avant l’activation du logon trigger.  
  - Partie des requêtes réécrites en fonctions pour faciliter les tests (voir `spyEvents`) et l'utilisation de ces fonctions dans `dbo.CompleterAudit`.
...
- **Version 2.6.0** :  
  ...
  - Optimisation de la gestion des connexions périmées. Ajout d’un mécanisme fail-safe : en cas de problème récurrent avec le logon trigger, celui-ci se désactivera automatiquement pour éviter toute interruption prolongée.  
  - Intégration d’une notification par email pour la procédure `dbo.CompleterInfoAudit`, avec ajout d’options dans `dbo.EnumsEtOpt` pour configurer l’adresse de destination et le serveur de messagerie.

---

- **Version 2.5.0** :  
  - Nombreuses corrections pour résoudre des problèmes rencontrés sous forte charge, ainsi que d’autres améliorations de résilience.

---

- **Version 2.1.2** :  
  - Ajout des événements `RPC_Completed`.

---

- **Version 2.1.1** :  
  - Allègement d’un traitement (modification mineure) et ajustements aux tests.

---

- **Version 2.1** :  
  - Renforcement de la résilience pour éviter la perte d’événements en cas d’interruption.  
  - Ajout d’un mécanisme empêchant l’insertion de doublons et mise en place d’un redémarrage automatique de la tâche toutes les 15 minutes en cas d’arrêt.  
  - Prise en charge des connexions multiples par session.  
  - Amélioration des outils de test pour simplifier la vérification de l’exactitude des résultats, garantissant notamment l’absence d’événements manquants.

---

- **Version 2.0** :  
  - Élimination du besoin de mémoriser dans une table, au niveau du déclencheur de login, les informations de connexion. Remplacement du mécanisme par l’utilisation d’un événement utilisateur contenant les informations nécessaires.  
  - Les événements sont désormais séquencés pour associer les requêtes à leur numéro de session et séquence d’événement. Cette méthode évite les difficultés liées à la mise en correspondance avec des données temporelles arrondies dans les fichiers trace.
